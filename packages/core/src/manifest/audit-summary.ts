...
function empty(): MutableTotals {
  return { requests: 0, ok: 0, errors: 0, unknown: 0, originalTokens: 0, forwardedTokens: 0, savedTokens: 0, potentialSavedTokens: 0, potentialCompressedItems: 0, upstreamInputTokens: 0, upstreamOutputTokens: 0, compressedItems: 0, redactedSecrets: 0, warnings: 0 };
}
...
function add(target: MutableTotals, manifest: AuditManifest) {
  ...
  target.potentialSavedTokens += manifest.potentialSavedTokens ?? 0;
  target.potentialCompressedItems += manifest.potentialCompressedItems ?? 0;
