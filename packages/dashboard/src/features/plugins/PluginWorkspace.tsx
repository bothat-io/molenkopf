...
function PluginRow(props: {
  ...
  return <CollapsibleGroup
    ...
    metrics={[
      ...
      { key: "actions", label: "Actions", value: actions.length || "none" },
  ...
  >
    <div className="plugin-panel-stack">
      ...
      <div className="plugin-panel-sections">
        ...
        <PluginWorkspaceSummary plugin={plugin} actions={actions} />
        <div className="plugin-panel-block">
          <div className="plugin-panel-heading">
            <h4>Actions and runtime state</h4>
            <p>Executable actions plus policy inheritance or runtime blocks.</p>
          </div>
          <div className="plugin-tag-list">
            <span className="plugin-tag soft">{team ? `Source: ${sourceLabel}` : "Source: Global default"}</span>
            {(actions.length ? actions : ["none"]).map((item) => <span key={item} className="plugin-tag">{item === "none" ? "No executable actions" : item}</span>)}
