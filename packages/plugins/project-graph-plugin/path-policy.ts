...
const DENIED_SEGMENTS = new Set([".git", ".aws", ".azure", ".claude", ".codex", ".docker", ".gnupg", ".kube", ".ssh", ".vscode", "appdata", "node_modules", "dist", "build", "coverage", ".molenkopf", "credentials", "secrets", "screenshots"]);
const DENIED_FILES = [/^\.env(?:\.|$)/i, /^\.credentials\.json$/i, /^auth\.json$/i, /\.(?:db|sqlite|pem|key|p12|log)$/i];
...
export function validateProjectRoot(rootPath: string): ScanWarning[] {
  ...
  const denied = isDeniedPath(basename(rootPath), defaultPolicy());
...
export function isDeniedPath(relativePath: string, _policy: ProjectGraphSettings): boolean {
  ...
  return lowerParts.some((part) => DENIED_SEGMENTS.has(part)) || DENIED_FILES.some((pattern) => pattern.test(parts.at(-1) ?? clean));
}
