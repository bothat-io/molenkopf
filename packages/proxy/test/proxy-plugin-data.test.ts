...
test("plugin data endpoints expose scoped compression data without query secrets", async () => {
  ...
  try {
    ...
    assert.equal(compression.snapshot.scope, "recent_audit_snapshot");
    assert.equal(compression.snapshot.limit, 200);
    assert.equal(compression.snapshot.observedRequests, 2);
    ...
    assert.equal(compression.metrics.providers.length > 0, true);
	    ...
	    assert.equal(compression.diagnostics.storageAvailable, true);
	    assert.equal(compression.diagnostics.skipReasons.some((item: { id: string }) => item.id === "no_long_json_string_candidate"), true);
	    assert.equal(compression.diagnostics.contentKindCounts.some((item: { id: string }) => item.id === "log"), true);
