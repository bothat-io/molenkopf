...
test("failed upstream attempts do not appear as a memory graph", async () => {
  ...
  try {
    ...
    const key = await issueKey(base, admin, "graph-failure");
    const response = await fetch(`${base}/v1/responses`, { method: "POST", headers: auth(key, { "content-type": "application/json" }), body: "{}" });
    ...
    assert.doesNotMatch(JSON.stringify(data), /POST \/v1\/responses|4xx|5xx|unauthenticated traffic|favicon/i);
  } finally {
    await proxy.close();
...
test("memory graph does not rebuild from request metadata after restart", async () => {
  ...
  try {
    const base = `http://127.0.0.1:${proxy.port}`;
    const admin = await setupAdmin(base);
    const key = await issueKey(base, admin, "graph-restart");
    await fetch(`${base}/v1/responses`, { method: "POST", headers: auth(key, { "content-type": "application/json" }), body: "{}" });
    await proxy.close();
    proxy = await startProxy({ port: 0, target: "http://127.0.0.1:9/v1", dataDir: dir });
    const nextBase = `http://127.0.0.1:${proxy.port}`;
    const nextAdmin = await loginAdmin(nextBase);
    const data = await fetch(`${nextBase}/__molenkopf/plugins/obsidian-graph-plugin/data`, { headers: { cookie: nextAdmin } }).then((item) => item.json());
    assert.equal(data.memoryGraph.nodes.length, 0, "graph is text-derived in memory, not rebuilt from audit metadata");
    assert.doesNotMatch(JSON.stringify(data), /POST \/v1\/responses|unauthenticated traffic|favicon/i);
  } finally {
