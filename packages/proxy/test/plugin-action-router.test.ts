...
test("generic plugin action route returns contract errors for missing, disabled, and actionless plugins", async () => {
  ...
  try {
    ...
    const noAction = await postAuth(env.base, "/__molenkopf/plugins/obsidian-graph-plugin/actions/run", {}, admin);
    ...
    const disabled = await postAuth(env.base, "/__molenkopf/plugins/obsidian-graph-plugin/actions/run", {}, admin);
